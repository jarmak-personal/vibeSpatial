...
class DeviceGeometryArray(ExtensionArray):
    ...
    def union_all(self, method="unary", grid_size=None):
        ...
        from vibespatial.dissolve_pipeline import union_all_gpu
    ...
    def representative_point(self):
        from vibespatial.representative_point import representative_point_owned
    ...
    def geom_equals(self, other):
        if isinstance(other, DeviceGeometryArray):
            from vibespatial.geometry_equality import geom_equals_owned
            return geom_equals_owned(self._owned, other._owned)
        return self.equals(other)

    def geom_equals_exact(self, other, tolerance):
        if isinstance(other, DeviceGeometryArray):
            from vibespatial.geometry_equality import geom_equals_exact_owned
    ...
    def __setstate__(self, state: tuple[list[bytes | None], Any] | tuple[list[bytes | None], Any, str]) -> None:
        from vibespatial.io_arrow import decode_wkb_owned
...
def _encode_owned_wkb_values(
    ...
    *,
    ...
    from vibespatial.io_arrow import encode_wkb_owned

    if all(buf.host_materialized for buf in owned.families.values()):
        return encode_wkb_owned(owned, hex=hex_output)

    try:
        from vibespatial.io_arrow import _encode_owned_wkb_column_device
...
def _dwithin_dispatch_distance(
    ...
    if query_family == PT:
        for tree_family in tree_families:
            if tree_family == PT:
                from vibespatial.spatial_nearest import _launch_point_point_distance_kernel
