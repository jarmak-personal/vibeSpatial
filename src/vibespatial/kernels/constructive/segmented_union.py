...
Groups are processed in parallel on CPU (Python-level parallelism over
groups, with GPU parallelism *within* each overlay_union_owned call).
A future iteration can batch multiple groups' pairwise overlays into a
single GPU launch, but the current overlay pipeline is strictly pairwise
(left[i] vs right[i]), so we iterate groups serially with GPU-accelerated
pair unions.
"""
...
def _segmented_union_gpu(
    ...
    *,
    ...
    # Concatenate all single-row results into one OwnedGeometryArray.
    return _concatenate_single_row_arrays(group_results)
...
def _concatenate_single_row_arrays(
    arrays: list[OwnedGeometryArray],
) -> OwnedGeometryArray:
    """Concatenate a list of single-row OwnedGeometryArrays.

    Materializes to Shapely for simplicity (small N = n_groups, not n_geoms).
    A future optimization can do buffer-level concatenation.
    """
    from vibespatial.geometry.owned import from_shapely_geometries

    all_geoms: list[object | None] = []
    for arr in arrays:
        geoms = arr.to_shapely()
        if geoms:
            g = geoms[0]
            all_geoms.append(g if g is not None else _EMPTY_POLYGON)
        else:
            all_geoms.append(_EMPTY_POLYGON)
    return from_shapely_geometries(all_geoms)
