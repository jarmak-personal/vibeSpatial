...
extern "C" __device__ inline void write_nan_bounds(double* out, int row) {{{{
...
extern "C" __global__ void scatter_mixed_bounds(
    ...
    const double* point_bounds,
    const double* linestring_bounds,
    const double* polygon_bounds,
    const double* multipoint_bounds,
    const double* multilinestring_bounds,
    ...
    try:
        for family, device_buffer in state.families.items():
            if device_buffer.bounds is None:
                device_buffer.bounds = runtime.allocate((geometry_array.families[family].row_count, 4), np.float64)
                temp_bounds.append((family, device_buffer, device_buffer.bounds))
                _launch_family_bounds_kernel(family, device_buffer, row_count=geometry_array.families[family].row_count, compute_type=compute_type)
