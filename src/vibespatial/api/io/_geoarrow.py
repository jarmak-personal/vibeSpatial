...
def construct_geometry_array(
    ...
    *,
    ...
    if len(shapely_arr) == 0 or mask.all():
        # Return an empty geoarrow.point array for empty/all-null geometry columns.
        ...
        if interleaved:
            coords_type = pa.list_(pa.field("xy", pa.float64()), 2)
        else:
            coords_type = pa.struct(
                [pa.field("x", pa.float64()), pa.field("y", pa.float64())]
            )
        null_arr = pa.nulls(len(shapely_arr), type=coords_type)
        field = pa.field(
            field_name, coords_type, nullable=True, metadata=extension_metadata
        )
        return field, null_arr
