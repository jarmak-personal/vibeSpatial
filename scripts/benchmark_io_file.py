...
def _suite_payload(*, suite: str, repeat: int) -> dict[str, object]:
    ...
    return {
        ...
        "metadata": {
            ...
            "notes": [
                "Runs the registered public io-file operation only.",
                "Internal direct-parser file component rails live in tests/test_io_benchmark_rails.py.",
            ],
        },
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Benchmark public file-format IO strategies.")
    parser.add_argument("--suite", choices=tuple(_SUITE_SCALES), default=None)
    parser.add_argument("--format", choices=_PUBLIC_FORMATS, default="geojson")
    ...
    if args.geometry_type != "point":
        raise SystemExit("Public io-file operation currently supports point fixtures only.")
