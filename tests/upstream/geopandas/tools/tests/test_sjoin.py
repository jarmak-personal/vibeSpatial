...
class TestSpatialJoin:
    ...
    @pytest.mark.parametrize("predicate", ["intersects", "contains", "within"])
    def test_outer(self, predicate, dfs):
        index, df1, df2, expected = dfs

        res = sjoin(df1, df2, how="outer", predicate=predicate)
        exp = expected[predicate].copy()
        exp["geometry"] = exp["geometry_x"].where(exp["geometry_x"].notna(), exp["geometry_y"])
        exp = exp.drop(["geometry_x", "geometry_y"], axis=1)

        if index == "default-index":
            exp["index_left"] = exp["index_left"].astype(float)
            exp["index_right"] = exp["index_right"].astype(float)
        if index == "multi-index":
            exp = exp.rename(
                columns={
    ...
    def test_sjoin_dwithin(
        ...
        # Sort by both index and index_right so the assertion is
        # insensitive to secondary ordering across implementations.
        sort_cols = [joined.index.name or "index", "index_right"]
        expected_sorted = expected_gdf.sort_index().reset_index().sort_values(sort_cols).reset_index(drop=True)
        joined_sorted = joined.sort_index().reset_index().sort_values(sort_cols).reset_index(drop=True)
        assert_frame_equal(expected_sorted, joined_sorted)
...
class TestSpatialJoinNYBB:
    ...
    def test_sjoin_outer(self):
        ...
        left = sjoin(self.pointdf, self.polydf, how="left")
        right = sjoin(self.pointdf, self.polydf, how="right")
        unmatched_left = int(left["index_right"].isna().sum())
        unmatched_right = int(right["index_left"].isna().sum())

        assert len(df) == len(left) + unmatched_right
        assert int(df["index_left"].isna().sum()) == unmatched_right
        assert int(df["index_right"].isna().sum()) == unmatched_left
