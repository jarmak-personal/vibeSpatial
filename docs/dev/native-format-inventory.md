...
| GeoJSON/Shapefile/native file read/write | parser output to owned geometry/native tabular payload; GeoJSON dynamic-size, validity, property type-count, unsupported-type, and family-domain scalar fences are runtime-counted and avoid raw CuPy host export; OSM standard layers use bounded nvCOMP/NVRTC byte decode, device attributes, indexed reference lookup and relation assembly; ordered collections use vector member-position compositions (see `docs/architecture/osm-pbf-native.md`); public GeoJSON string, Python geo-interface, geo-dict, and vector-file exports are terminal native export boundaries for native-backed objects; read-file datetime compatibility rewrites, lazy GeoJSON property expansion, GeoJSON mask-filtered Arrow/WKB reads, and named-index vector-file writes preserve deferred attributes until the terminal export | `NativeFrameState` with deferred attributes | format metadata fences; public export only; GeoJSON/geo-dict/vector-file exports are terminal `NativeExportBoundary` events | covered |
| WKB/WKT/GeoArrow encode/decode | owned buffers to Arrow/WKB bridge; public Arrow/Feather readers attach metadata-seeded `NativeFrameState`; native tabular, low-level owned, and public host-owned WKB/GeoArrow exports encode from owned buffers instead of temporary GeoSeries/public arrays; every WKB field carries the GeoArrow extension name and metadata; empty, all-null, nullable, byte-string, and invalid one-point LineString inputs preserve validity and `on_invalid` policy without fallback; direct GeometryCollection WKT ingress is an observable public compatibility boundary because GeometryCollection is not a concrete owned family; unsupported low-level and native-tabular GeoArrow geometry mixes use the repo-owned WKB bridge instead of upstream WKB constructor fallback, while public GeoSeries/GeoDataFrame unsupported mixed GeoArrow exports preserve GeoPandas `ValueError`; GeoSeries, GeoDataFrame, and direct native-tabular Arrow exports record terminal native export boundaries only after successful export; device GeoArrow encode misses are observable before host terminal export and strict-native rejects them; WKB decode sizing probes use named scalar fences; device GeoArrow family, validity, point-empty gates, and supported single/multi promotions use owned structure proofs without runtime D2H; pylibcudf GeoArrow decode reuses column child sizes before named scalar fences | terminal `NativeExportBoundary` plus metadata carrier | current coordinate storage is 2D: XYZ, M, and ZM codecs decline observably rather than discard dimensions; export unless immediately consumed; native consumer handoff covered for public Arrow/Feather ingress; scalar fences and explicit Arrow/GeoParquet/Feather/Shapely/display/GeoJSON/WKB/WKT exports are runtime events | covered for 2D geometry families, null/empty/invalid policy, and GeoArrow metadata; dimensional codecs are intentionally unsupported |
