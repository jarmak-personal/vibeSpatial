...
| Full suite / profile | The final isolated per-query medians sum to VS 553.63 s versus optimized GPD 8,086.00 s, or 14.605x. The final repeat-3 10K shootout is 14/14 exact with zero fallback/offramps: VS totals 2.664 s versus GPD 3.537 s. Restoring site suitability's transit-buffer semijoin makes that workflow 4.07x faster than GPD; the other thirteen workflows change +0.50% in aggregate with a +2.05% worst result, and the full suite changes +1.55% despite the added work. Final-worktree isolated 1M strict-native runs close all four former capacity failures: redevelopment 403.90 s, retail 7.06 s, corrected site suitability 4.13 s, and transit 60.54 s. Site suitability returns 350,223 rows / 335,170.62 hull area while GeoPandas remains in `sjoin` at the 895-s timeout; the other three preserve their exact optimized-GPD fingerprints. Transit succeeds under the default capped pool, although its separate statement-level trace replay retains enough instrumentation state to exceed 24 GiB. Existing admitted 1M workflows retain the prior <=1.77% worst regression. The post-fix mandatory full profile reports zero compute transfers, zero compute materializations, and zero fallbacks for every successful 1M pipeline; the slowest stage is `predicate-heavy/read_geojson` at 71.56 ms, so no stage crosses the 1-s CPU-heavy investigation threshold. |
