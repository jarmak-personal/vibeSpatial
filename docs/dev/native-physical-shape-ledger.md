...
| Many/few overlay constructive | Candidate generation, refinement, constructive geometry, and projection remain native. Grouped difference uses one full-source `NativeGrouped`; signed winding deltas survive partial-overlap renoding; exact cycle orientation, fixed-capacity indexed containment, and O(E) boundary peeling replace host probes and quadratic face relations. Lower-dimensional remnants remain in `NativeGeometryComposition` until export. | `NativeSpatialIndex`/metadata to candidate `NativeRelation`, refine relation, constructive provenance, native row or attribute projection. | `NativeSpatialIndex`, `NativeGeometryMetadata`, `NativeRelation`, left/right `NativeFrameState` | `NativeTabularResult` with `NativeGeometryProvenance` | Explicit overlay export or IO write | Relation constructive, grouped complement, boundary composition, indexed view, DE-9IM, equality, and exception-atomicity canaries | Focused overlay/dissolve verification passes 504 tests. The mandatory profile has zero compute materializations/fallbacks and 11 bounded planning D2H packets totaling 8,376 bytes. The fresh 10K wall-time regression keeps this row active until exact collective construction returns to the rich floor. |
| Grouped geometry reduce | `NativeGrouped` feeds constructive work through sorted offsets. Buffered-line dissolve now keeps leaf fan-in bounded at 32, uses binary aggregate levels, and continues the final cross-partition merge through the same bounded tree instead of one unbounded topology row. Other grouped complement, repair, paging, component, and microcell carriers remain capacity-backed. | Segmented reduction plus bounded fan-in, fragments, sparse rowsets, capacity-backed buffers, streamed classification, and exact boundary atoms. | `NativeGrouped`, `NativeDeviceSelection`, `NativeGroupedSelection`, `NativeFrameState`, `NativeGeometryMetadata`, `NativeGeometryComposition` | `NativeGrouped` result, geometry composition, or native tabular output | Dissolve/grouped export and terminal GeometryCollection assembly | Buffered-line bounded-tree guard plus grouped topology/reducer/global-union/residual canaries | The 1M vegetation deferred exact union improves from 65.40s to 34.32s with the same valid native shape (183,584 coordinate lanes and 45,896 ring lanes). This remains an active large-constructive scaling target; prior sub-100ms grouped canaries stay green. |
