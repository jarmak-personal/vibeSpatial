...
- row-position, pairwise/relation index, host-array, grouped export, cleanup-mask, spatial index/query/nearest/KNN/distance/segment exports, explicit selected-face debug export, lower-level topology export, expression assignment, device-label, and owned-geometry materialization are explicit runtime-counted boundaries
